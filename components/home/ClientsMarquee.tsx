  ...
  return (
    <section id="clients" ref={containerRef} className="pt-12 pb-24 bg-(--surface) border-y border-(--border) overflow-hidden">
      <div className="container mx-auto px-6 mb-24">
        <div className="relative overflow-hidden rounded-[2rem] bg-black/10 border border-white/5 py-12">
          {/* Enhanced Fade edges */}
          <div className="absolute left-0 top-0 bottom-0 w-64 z-10 bg-linear-to-r from-(--surface) via-(--surface)/80 to-transparent pointer-events-none" />
          <div className="absolute right-0 top-0 bottom-0 w-64 z-10 bg-linear-to-l from-(--surface) via-(--surface)/80 to-transparent pointer-events-none" />
            ...
            <motion.div style={{ x: x1 }} className="flex">
              <div className="flex animate-marquee gap-0 min-w-max">
                {[...row1, ...row1, ...row1, ...row1].map((client, i) => (
                  <div
                    key={`row1-${i}`}
                    className="shrink-0 flex items-center px-16 border-r border-white/5 group cursor-default gap-8"
                  >
                    {client.logoUrl && (
                    ...
                    <span className={`text-white text-lg font-black uppercase whitespace-nowrap group-hover:text-[#F58220] transition-colors duration-300 ${isAr ? 'tracking-normal' : 'tracking-[0.2em]'}`}>
