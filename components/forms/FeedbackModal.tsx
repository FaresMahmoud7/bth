...
import { validatePhoneNumber, COUNTRY_CODES } from "@/lib/validation";
            ...
            >
              ...
              <div className="p-8 border-b border-white/5 flex items-center justify-between">
                ...
                <button
                  ...
                  className="p-2 text-white/20 hover:text-white transition-colors rounded-full hover:bg-white/10"
                >
                  <X className="w-6 h-6" />
                </button>
              </div>
              ...
              <div className="p-8 max-h-[70vh] overflow-y-auto">
                <form onSubmit={handleSubmit} className="space-y-8">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <select
                          ...
                          className="w-[90px] h-14 px-2 rounded-xl bg-white/5 border border-white/10 text-white outline-none focus:border-[#F58220] transition-all cursor-pointer text-sm"
                        >
                          {COUNTRY_CODES.map((c) => (
                        ...
                        <input
                          ...
                          placeholder={countryCode === "+966" ? "05xxxxxxxx" : "xxxxxxx"}
                          className={`flex-1 h-14 px-5 rounded-xl bg-white/5 border ${phoneError ? "border-red-500" : "border-white/10 focus:border-[#F58220]"} outline-none transition-all text-white placeholder:text-white/10`}
                        />
                      </div>
                      {phoneError && (
                        <p className="text-red-400 text-xs mt-1.5">
                          {phoneError.includes(" / ")
                            ? phoneError.split(" / ")[isAr ? 0 : 1]
                            : phoneError}
                        </p>
                      )}
                    </div>
                  ...
                  <button
                    type="submit"
                    disabled={isSubmitting || isSuccess || !!phoneError}
                    className={`w-full h-16 rounded-full font-bold text-sm uppercase tracking-widest flex items-center justify-center gap-3 transition-all ${isSuccess ? 'bg-green-600 text-white' : 'glow-button-primary'} disabled:opacity-60 disabled:cursor-not-allowed`}
                  >
                    {isSubmitting ? (
